...
(same pattern as Matrix dual-screen: game binds GAME_BIND_PORT, GUI binds GUI_BIND_PORT).
"""
...
EVILEYE_UDP_GAME_BIND = 4627
EVILEYE_UDP_GUI_BIND = 7802
...
def cleanup_evileye_game(ctx: DualRuntimeCtx) -> None:
...
class EvilEyeGameDisplays:
    def __init__(
        ...
        min_players: int = 2,
        max_players: int = 8,
        valid_player_counts: list[int] | None = None,
    ):
        self.root = root
        self.ctx = ctx
        ...
        self.valid_player_counts = valid_player_counts or list(
            range(min_players, max_players + 1)
        )
        self._ui_scale = 1.0
    ...
    def _setup_control_panel(self, control_title: str, parent: tk.Widget):
        ...
        default_count = self.valid_player_counts[0]
        self.players_var = tk.IntVar(value=default_count)
        self.player_buttons = []
        btn_frame = tk.Frame(parent, bg=self._panel_bg)
        btn_frame.pack()
        for count in self.valid_player_counts:
            btn = tk.Button(
                btn_frame,
                text=str(count),
                ...
                height=1,
                cursor="hand2",
                command=lambda num=count: self._select_players(num),
            )
            self._style_player_btn(btn, count == default_count)
